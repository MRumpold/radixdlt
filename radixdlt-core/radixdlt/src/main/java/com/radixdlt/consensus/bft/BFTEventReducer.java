...
public final class BFTEventReducer implements BFTEventProcessor {
	...
	public void processVote(Vote vote) {
		...
		switch (this.pendingVotes.insertVote(vote, this.validatorSet)) {
			case VoteAccepted voteAccepted -> log.trace("Vote has been processed but didn't form a quorum");
			case VoteRejected voteRejected -> log.trace("Vote has been rejected because of: {}", voteRejected.getReason());
			case QuorumReached quorumReached -> {
				this.hasReachedQuorum = true;
				viewQuorumReachedEventDispatcher.dispatch(new ViewQuorumReached(
					quorumReached.getViewVotingResult(),
					vote.getAuthor()
				));
			}
		}
